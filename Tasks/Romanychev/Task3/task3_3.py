"""
Vowel and Word Counter with Language Detection Script
This script allows you to input a text string either from the keyboard or a file and detect its language.
It then counts the number of vowels and words in the input string based on the detected language and plots a histogram
to visualize the distribution of vowel counts across supported languages.
Additionally, it displays the word counts for the input string along with the 10 most frequent words, and the total number
of unique words in the input.

Usage:
1. Choose the source of the input string:
    - If you choose '1', you can input a string from the keyboard.
    - If you choose '2', the script will attempt to read a text file ('test(war and peace)_task3_1.txt')
      and detect its language.

2. If you input the string from the keyboard:
    - You will be prompted to choose the language of the input string from the following options:
        'en' - English,
        'ru' - Russian,
        'pl' - Polish,
        'de' - German,
...
3. The script will detect the language of the input string and count the number of vowels and words in it based on the chosen language.

4. A histogram will be plotted, showing the distribution of vowel counts for the selected language.
...
Note: The script uses the 'langdetect' library for language detection. Please make sure to install it using 'pip install langdetect'
